...
sequenceDiagram
    ...
    Dev->>ACL: ③ bisheng 编译 kernel 源码 → 二进制(.o)<br/>ascendc_library 打包 kernel 产物
    end
    ...
    Host->>ACL: ⑦ aclrtlaunch_*(handle, args)<br/>⚠️ 只是"下发任务"，CPU 立刻返回
...
| 编译 | 编译器 | bisheng 编译 + ascendc_library 打包 | 只做一次 |
...
- 华为昇腾 · CANN 社区版 Ascend C 算子开发指南（Kernel 直调 / ACLRT_LAUNCH_KERNEL / stream）：
