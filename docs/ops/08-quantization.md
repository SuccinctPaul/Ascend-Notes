...
```
scale = 127 / amax                 # 每个张量（或逐行/逐通道）一个缩放
X_q = clamp( round( X / scale ), -127, 127 )     # 量化 Quant
X̂   = X_q * scale                  # 反量化 Dequant（还原到原数值范围）
```
...
常见的 "W8A8" 表示**权重（Weight）是 8bit、激活（Activation）也是 8bit**。这样矩阵乘 `activation @ weight` 的两个操作数都是 INT8，硬件可以用 INT8 的 MAC（如在 Cube 上加速），把乘法做满吞吐。风格：

```
ScaleA × Q_a  ·  weight_adjust的乘积 ≈ 原乘积（逐 block 的 scale 设计见下）
```
...
从 FP16 到 INT8/FP8，元素字节数从 2 → 1，模型权重和 KV Cache 体积减半；一个 70B 模型从"塞不进"变成"塞得进"一块卡。从 FP32 → INT8 能省 4×。

### 3.2 算力 3 倍速左右（取决于硬件）

INT8 / FP8 的矩阵乘吞吐往往是同卡 FP16 的三倍上下。对大模型以 GEMM 为主的推理，这就是实打实的加速（GPTQ 论文里报告端到端 3~4 倍速）。
...
def quant_int8(x, amax):
    scale = 127.0 / float(np.abs(amax) if amax > 0 else 1e-12)
...
这和仓库 CONTEXT.md 的**混合精度**（fp16 输入输出 + fp32 累加器）是同一思想——**"算的时候用宽账本，存的时候用窄格式"**。
...
设某权重块 `[0.5, 1.5, -2.0]`，`amax=2.0`：
- `scale = 127/2.0 = 63.5`；
- `0.5/63.5→0.008→round≈0`，`1.5/63.5→0.0236→round≈0`，`−2.0/63.5→−0.0315→round≈0`……这暴露了问题：**整张一个 scale 时，小数值全被压成 0**（除了最大那个）。

这正是"**按块 scale**（block-wise）更好"的直观理由：把块分小，每个块里范围更窄，`scale` 更贴合，量化误差更小。代价只是 Dequant 时要给每块乘回不同的 scale（多几次乘）。

```mermaid
flowchart LR
    A["权重块 amax=2"] -->|"scale=63.5"| B["0.5,1.5,−2 → 0,0,−127 ≈ 全被压没"]
    C["按小块 scale：块内 amax 更小"] -->|"块内 scale 更贴"| D["小值也保住，误差更小"]
```
...
- **fp16 + fp32 累加器**：输入可能是 fp16 或更低的量化格式，但**求和必须在累加器（宽格式）里做**，避免 K 维累积把小数进位丢掉。σ 越大越需要宽累加器；
