...
```
人话总结：element-wise 就是"两张数表按格子一对一地算"；
...
在 Ascend C 里写一个 element-wise kernel，沿用的是统一的"搬运+计算+搬运"模板（这正是 CONTEXT.md 里说的 **数据流**）：
...
需要注意的是：因为"先搬进新块、再算块"有时间差，多核 / 多缓冲下的同步要靠**显式的同步（barrier / EnQue-DeQue）**来保证"数据到齐了才开算"，这正是 CONTEXT.md 里说的"每一步搬运都是显式发起 + 同步"。
...
最典型的融合是把激活/归一化作为 GEMM 的 **尾部操作（epilogue）**：Cube 把矩阵乘结果累加到 **L0C** 后，先用 DMA 把 L0C 跨域搬到 **UB**，再接反 ReLU/缩放，最后一次性写回 GM。这样 GEMM 结果不落 GM，直接就地加工——这就是 CONTEXT.md 图 B 里"⑦→⑧ 可选 Vector 加工通路"的来历。
...
4. **动手自测（可选）**：把 `Add → Mul` 两段写成"融合成一个 kernel"，用本仓库的 `verify()`（atol=rtol=1e-2）对比融合前后的输出是否一致，再量耗时——亲眼看"少一趟 GM 往返"换来多少收益。
...
5. **NPU 的 L1 是硬件自动 cache 吗？** → 不是，它由 **kernel 代码显式管理**（CONTEXT.md 反复强调的点）。
