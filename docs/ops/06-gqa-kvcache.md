...
```
人话总结：KV Cache 是把算过的钥匙(Key)/值(Value)存起来别再重算；
...
```
KV 大小 ≈ 2 × 层数 × (K/V头数) × 每头维度 × 序列长度 × 字节/元素
```
...
把 KV Cache 存在 GM 时，要按"层数 × KV头数 × 维 × 序列上限"算好预留量。GQA 让这条公式里的"KV头数"缩小，同样的物理显存能支撑更长的上下文或更大的 batch。推理部署时常把 KV Cache 单独分配、做内存池复用，避免碎片。
...
对已缓存的历史 K、V，NPU kernel 只做**读**并把新 token 的 K、V **写追加**。这一"读多写少"的模式，正是让 `L1→L0A/B` 复用和 Cube 打满的关键——配置好 LT 预取、双缓冲，读historical 的带宽就不会浪费。
...
4. **"GQA 的 Q 头变少了会影响表达能力吗？"** 会略降（相比 MHA），但 GQA 用**更多的 head 维**（每个 Q 头投影更宽）补偿，GQA 论文在常见规模上报告与 MHA 几乎同等精度。它换来的是逻辑/带宽的大幅下降，属于"少省换快"的划算买卖。
...
- MHA（每头自有 K 与 V，共 `H=32` 头）：`2·32·32·128·4096·8·2` ≈ **8.6 GB / 批**（示意量级）；
- GQA（`G=4`，KV 头数变 `H_g=H/G=8`）：KV Cache 大约变成上述的 **1/4** ≈ 2.1 GB / 批。
