...
沿用本仓库 CONTEXT.md 的术语，这里再加 4 个性能术语：

| 术语 | 含义 | 对应 CONTEXT.md 的类比 |
|---|---|---|
...
**Bytes（需要从 GM/HBM 搬多少字节）**——朴素实现下，A、B、C 三个矩阵都要进出 GM：

```
输入 A: M×K 个 fp16 = M×K×2 字节
输入 B: K×N 个 fp16 = K×N×2 字节
输出 C: M×N 个 fp16 = M×N×2 字节
朴素 Bytes ≈ 2×(MK+KN+MN)
128³ 朴素 Bytes = 2×(128² + 128² + 128²)×2 = 2×3×16384×2 = 196,608 B ≈ 192 KB
```

**I = FLOPs ÷ Bytes**：
```
朴素 128³ 的 I = 4,194,304 ÷ 196,608 ≈ 21.3 FLOP/B
```

**人话**：这个 I ≈ 21，意思是"从 HBM 里每搬 1 字节，只换来 21 次浮点运算"。这个数偏小 → 大概率是**带宽受限**。（对比：K 越大、每个 tile 在片上复用越久，I 越高，就越接近计算受限。）
...
- **带宽墙（斜线）**：`B (bytes/s)` 也就是 `B × I`，斜率为带宽的那条斜线。
...
**人话**：I_ridge 就是"拐点"。你的 I 比拐点小，就是带宽说了算；比拐点大，就是算力说了算。
...
- **Q：为什么我对 910B 算出来的 I_ridge 和文档不一样？** 峰值 P、带宽 B 随型号/频率/实测口径变化很大，**务必用目标芯片的实测值**（标"待核验"即提醒你别照搬）。
...
| 带宽 B | 硬件最多一秒搬多少字节 |
| 山脊点 I_ridge = P/B | "带宽受限"和"计算受限"的分界拐点 |
