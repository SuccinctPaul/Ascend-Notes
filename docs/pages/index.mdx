...
**面向 0→1 的昇腾 Ascend NPU 核心开发手册（原 ascend-notes）**

从硬件架构到四种 DSL 开发，从 LLM 算子优化到 Profiling 实战——
一份文档，帮你从零成长为专业的 NPU kernel 开发者。

[快速开始](/getting-started) · [从硬件架构开始](/hardware/01-ai-core-overview) · [四种 DSL 总览](/dsl/00-dsl-overview) · [术语表](/reference/context) · [GitHub](https://github.com/SuccinctPaul/ascend-handbook)

---

## 快速开始

有 910B 机器在手?三条命令跑通第一个 NPU kernel(完整步骤见 [快速开始](/getting-started)):

```bash
sudo ./scripts/install_npu_toolchain.sh          # ① CANN 工具链
./scripts/install_dsl_envs.sh all                # ② 四种 DSL 环境
cd examples/python && uv run python src/gemm.py  # ③ 第一个 kernel(CPU 基准,NPU 版见快速开始)
```
...
- **六卷一站式手册**：硬件架构 → 性能模型 → 四种 DSL 核心手册 → LLM 算子实战 → 前沿方案综述 → 构建部署与术语表。
...
flowchart LR
    ...
    E1["部署 / 环境搭建"] --> E
    F1["FA3 / MLA / FP8 / MoE / ATB / vllm-ascend"] --> F
```
...
| [02 · RMSNorm](/ops/02-rmsnorm) | 均值归一化、Vector reduction、精度处理 | ✅ 2026-09 实测（见 §八） |
| [03 · Softmax](/ops/03-softmax) | 三阶段 reduction、数值稳定、UB 内完成 | ✅ 实测（见 §八） |
| [04 · RoPE 旋转位置编码](/ops/04-rope) | 旋转矩阵、复数乘法、Vector 实现 | ✅ 2026-09 实测（见 §八） |
| [05 · GELU 与激活](/ops/05-gelu) | tanh 近似、逐元素、融合到 GEMM epilogue | ✅ 实测 |
| [06 · GQA 与 KV Cache](/ops/06-gqa-kvcache) | 分组注意力、KV Cache 管理 | ✅ 2026-09 实测（见 §八） |
| [07 · FlashAttention](/ops/07-flash-attention) | 分块注意力、IO 复杂度优化 | ✅ 2026-09 实测（见 §八） |
| [08 · 量化与反量化](/ops/08-quantization) | INT8/fp8 量化、Cube 翻倍算力 | ✅ 2026-09 实测（见 §八） |
| [09 · GEMM（四种 DSL 实证）](/ops/09-gemm) | 矩阵乘复用、tiling、Cube 16×16 MAC | ✅ 实测 |

---

## 第 06 卷 · 前沿方案（学术与产业）

截至 2026-09 的学术/产业最新方案综述，每篇回链正卷原理与可运行代码。

| 篇目 | 内容 |
|---|---|
| [00 · 前沿方案总览](/sota/00-sota-overview) | 全景地图、阅读路径、与正卷的对应关系 |
| [01 · Attention 前沿](/sota/01-sota-attention) | FlashAttention-3、MLA/FlashMLA、稀疏注意力 NSA→DSA |
| [02 · 量化前沿](/sota/02-sota-quantization) | FP8 细粒度缩放、DeepGEMM、W4A8、FP4 |
| [03 · MoE 与解码前沿](/sota/03-sota-moe-decode) | fused MoE、DeepEP、MTP/投机解码、superkernel、P/D 分离 |
| [04 · 编译器与 DSL 前沿](/sota/04-sota-compiler) | Triton/TileLang/CuTe、自动 warp specialization、agent 调优 |
| [05 · 昇腾产业实践](/sota/05-sota-ascend) | ATB/MindIE、vllm-ascend 自定义算子链路、CloudMatrix384 |
...
（早期 GEMM 首跑在 `vllm-hust-cyj-21rc-cloud-piou` 容器，环境配置相同）。
- 架构：aarch64 (Ubuntu)
- CANN：9.0.0
- NPU：Ascend910B 系列
...
```bash
# 本地预览文档 (Vocs, 详见 [构建与部署说明](/deployment))
