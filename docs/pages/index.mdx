...
- **五卷一站式手册**：硬件架构 → 性能模型 → 四种 DSL 核心手册 → LLM 算子实战 → Profiling 与调优。
...
- 每篇文档统一骨架：TL;DR + Background + Why + 正文 + 图表 + FAQ + 末尾汇总 + Reference。
...
| 篇目 | 内容 |
|---|---|
| [构建与部署说明](/deployment) | 环境要求、CANN 配置、运行方式 |
