...
本讲术语以[术语表](/reference/context)「两级系统」词条（host / device / tiling 参数 /
异步提交 / 同步 / kernel handle / ACL 运行时）为权威依据，全文严格对齐。
...
CPU 流水与 NPU 流水就这样咬合起来轮流前进，吞吐肉眼可见地提升。
...
sequenceDiagram
    ...
    Dev->>Dev: ③ bisheng 编译 kernel 源码 → 二进制(.o)，ascendc_library 打包 kernel 产物
    end
...
| 验证        | 你      | 与 CPU 参考基准比对                     | 对答案，方法见 [Python 基准](/dsl/01-python-baseline) |
...
> 拼写口径：`aclrtlaunch_*` 是 bisheng 依据核函数**自动生成**的启动入口（`*_` 是核函数名后缀）；`ACLRT_LAUNCH_KERNEL` / `aclrtLaunchKernel` 是 ACL 运行时的**运行时 API**（前者是后者的宏封装）。三者最终都走"投递任务单"这条语义。
> aclnn 是 CANN 提供的高层算子库 API（每个算子一个 `aclnnXxxGetWorkspaceSize` + `aclnnXxx` 两段式接口），由框架调用；自己写的算子要走 aclnn 需注册进算子库。
...
对照生命周期表：2≈load、4≈launch、5≈同步、6≈取结果。**最容易漏掉的是第 5 步（同步）**——不同步就读数据，是新手最常见的"偶发旧数据"bug 来源。
...
> 以下链接均为昇腾官方来源（撰写时可访问；文档站随版本改版，若失效请从 CANN 文档中心首页进入。链接锚定的 CANN 版本号可能与本仓库实测的 9.0.0 不同，以内容为准）。

- [华为昇腾 · CANN 社区版 Ascend C 算子开发指南](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/81RC1alpha001/devguide/opdevg/ascendcopdevg/atlas_ascendc_10_0051.html)（Kernel 直调 / ACLRT\_LAUNCH\_KERNEL / stream）

- [华为昇腾 · AscendCL API 参考 aclrtSynchronizeStream](https://www.hiascend.cn/document/detail/zh/canncommercial/80RC2/apiref/appdevgapi/aclcppdevg_03_0075.html)（阻塞等待 stream 任务完成）

- [华为昇腾 · AscendCL API 参考 aclrtLaunchKernel](https://www.hiascend.cn/document/detail/zh/CANNCommunityEdition/80RC3alpha001/apiref/appdevgapi/aclcppdevg_03_0145.html)（异步下发、必须同步）

- [华为昇腾 · Ascend C 编程模型概述](https://asc.gitcode.com/guide/编程指南/编程模型/编程模型概述.html)（host/device 两级异构体系）

---

## 上一篇 / 下一篇

- [02 · 存储层级与访问权域](/hardware/02-storage-hierarchy)
- [04 · 昇腾 vs Hopper vs Gaudi](/hardware/04-npu-architecture-comparison)
