...
本讲术语以[术语表](/reference/context)的存储类词条（GM / 片上缓冲 / L1 / L0C /
L0A-B / UB / DMA / 数据流）为权威依据，全文严格对齐。
...
GM 访问次数能降下来好几个数量级——这就是“优化前后 GM 访问量差 N 倍”的硬件根源（量化方法见[性能模型与 Roofline](/perf/01-roofline-perf-model)）。
...
- **补充（进阶）**：Cube 还有一条把 L0C 结果直接写回 GM 的"落定"通路（fixpipe），部分 DSL 会用它省掉 UB 中转（见 [TileLang 篇](/dsl/03-tilelang-ascend) 的 `T.copy(L0C→GM)`）。本卷教学模型以"L0C→UB→GM"为主线，知道有这条捷径即可。
...
2. **Cube 和 Vector 不是同一个引擎**：一个管矩阵乘、一个管逐元素，谁也代不了谁；
   结果要在 L0C 与 UB 之间经 DMA 搬来搬去——这就是“跨域搬运税”。
...
flowchart LR
    A1["① 数据躺在 GM"]
    A2["② DMA GM→L1: 单核私有直达; 跨核才经片上共享存储中转"]
    A3["③ L1 灌入 → L0A / L0B (Cube 域输入)"]
    A4["④ Cube 取数计算 16×16"]
    A5["⑤ 结果累加到 L0C (fp32)"]
    A6["⑥ 若需 Vector 加工: DMA 跨域 L0C → UB"]
    A7["⑦ Vector 在 UB 上做 ReLU 等"]
    A8["⑧ DMA 搬回 GM"]
...
- ② 的跨核分支是“可选”：单核私有数据直达 GM→L1；确实要跨核才绕共享存储。
...
**错。** L1/L0/UB 是软件显式管理，不是硬件自动 cache——`T.copy` / `DataCopy` 不写，数据就不到位。这是和 CPU 最本质的分界线。
...
**错。** 跨核数据要经片上共享存储（中转站）中转，不能核间直取；纯核内私有数据才直达 GM → L1。
> 注：跨核通路的确切硬件形态官方资料较少，“经共享存储中转”是本仓库采用的教学模型，细节待核验。
...
6. 跨核数据要经片上共享存储中转（教学模型，待核验）；纯核内私有则 GM→L1 直达。
...
- [华为昇腾 · C++ Tensor 编程概述](https://asc.gitcode.com/guide/编程指南/编程模型/AI-Core-SIMD编程/基于Tensor的CPP编程/CPPTensor编程概述.html)（内存层级、LocalTensor/GlobalTensor）
- [华为昇腾 · Ascend C 编程模型概述](https://asc.gitcode.com/guide/编程指南/编程模型/编程模型概述.html)（AI Core 硬件基础、存储与访问权域）
- [华为昇腾 · CANN 社区版 Ascend C 算子开发指南](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/81RC1alpha001/devguide/opdevg/ascendcopdevg/atlas_ascendc_10_0051.html)（Kernel 实现/数据搬运）

---

## 上一篇 / 下一篇

- [01 · AI Core 硬件模型全貌](/hardware/01-ai-core-overview)
- [03 · host/device 与 kernel 生命周期](/hardware/03-host-device-kernel-lifecycle)——存储看完了，看“谁指挥这些搬运”。
