...
前三篇我们看清了昇腾的 AI Core、存储层级与 host/device 分工。把它放到一个更大的坐标系里，
...
> **关于数字的说明（红线）**：本表的型号数字，凡标注（官方）的均来自文末官方链接；
...
L1/L0/UB、Gaudi 的 VLM——TPC 核内的 Very Local Memory），这直接决定了编程时“你要操多少心”。这是**新手最容易
混淆、也是性价比最高的知识增量**。
...
| **搬运机制** | DMA 引擎，显式搬运，跨域必须经 DMA | TMA（Tensor Memory Accelerator）+ LDGSTS，支持异步批量搬运 | TPC 的 load/store 槽位（自带地址计算单元 AGU）+ 片上 DMA + RoCE 网卡（跨卡 RDMA）；硬件 cache |
...
- **Gaudi**：片上 96 MB SRAM 可按 L3/L2 用，是硬件 cache，但可用 **cache directives**
  （No-$、L2$、L3$、L2$+L3$）手动指点；TPC 里还有一块显式的 **VLM（Very Local Memory，80 KB）** 供程序直接用。
...
| Gaudi 3 | MME ×8（每 MME 256×256 MAC 阵列，fp32 累加，官方；全芯片合计 MAC 数官方未给统一口径，待核验） | TPC ×64，VLIW SIMD |
...
- **Gaudi**：TPC 的 load/store 指令自带地址计算（AGU，Address Generation Unit）+ 片上 DMA；跨卡搬运走 RoCE 网卡（RDMA over Converged Ethernet）。
...
| 问题 | 昇腾 | H100 | Gaudi |
| --- | --- | --- | --- |
| 最外层大仓库叫什么、多大？ | GM=HBM（容量待核验） | HBM3 80GB，>3 TB/s | HBM2e 128GB，3.7TB/s |
| 片上有没有“硬件自动”的缓存？ | 显式管理为主（自动 cache 机制官方未披露，待核验） | 有（L2 自动） | 有（L2/L3 自动，可指令控制） |
...
对比只是起点——每家的峰值算力怎么算、Roofline 怎么用，见 [如何计算 NPU 算力](/perf/00-npu-peak-flops-calculation) 与 [性能模型与 Roofline](/perf/01-roofline-perf-model)。

### 4.8 给你的学习路径建议

- 想快速上手昇腾 GEMM：先吃透 [01 · AI Core 硬件模型](/hardware/01-ai-core-overview)和 [02 · 存储层级](/hardware/02-storage-hierarchy)的**访问权域 + 唯一 DMA**，这是昇腾的“命门”。
...
> 以下链接均为各芯片厂商官方来源（撰写时可访问；若失效欢迎提 issue）。

**华为昇腾（官方）：**
- [Ascend C 编程模型概述](https://asc.gitcode.com/guide/编程指南/编程模型/编程模型概述.html)（AI Core 硬件基础）
- [C++ Tensor 编程概述](https://asc.gitcode.com/guide/编程指南/编程模型/AI-Core-SIMD编程/基于Tensor的CPP编程/CPPTensor编程概述.html)（内存层级）

**NVIDIA（官方）：**
- [NVIDIA Hopper Architecture In-Depth](https://developer.nvidia.com/blog/nvidia-hopper-architecture-in-depth/)（H100 HBM3 80GB / >3TB/s、Tensor Core、TMA）
- [NVIDIA Hopper Tuning Guide](https://docs.nvidia.com/cuda/pdf/Hopper_Tuning_Guide.pdf)（shared memory 228KB、TMA、SM 结构）
- [NVIDIA H100 GPU 官方产品页](https://www.nvidia.com/en-us/data-center/h100/)

**Intel / Habana（官方）：**
- [Intel Gaudi Architecture](https://docs.habana.ai/en/latest/Gaudi_Overview/Gaudi_Architecture.html)（Gaudi 3：128GB/3.7TB/s、8 MME、64 TPC、96MB SRAM）
- [Intel Gaudi 3 AI Accelerator 技术白皮书](https://cdrdv2-public.intel.com/817486/gaudi-3-ai-accelerator-white-paper.pdf)（256×256 MME、L2/L3、cache directives、VLM）
- [Intel Gaudi 3 Hot Chips 2024](https://hc2024.hotchips.org/assets/program/conference/day1/60_HC2024.Intel.RomanKaplan.Gaudi3-0826.pdf)（内存子系统、MME 256×256、TPC VLM/寄存器）

---

## 上一篇

[03 · host/device 与 kernel 生命周期](/hardware/03-host-device-kernel-lifecycle)——硬件卷到此收官，接下来进入[性能模型](/perf/00-npu-peak-flops-calculation)与[四种 DSL 手册](/dsl/00-dsl-overview)。
