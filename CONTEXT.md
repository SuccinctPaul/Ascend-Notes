...
flowchart LR
    ...
    A7["⑦ 若需 Vector 加工:<br/>DMA 跨域 L0C → UB"]
...
sequenceDiagram
    ...
    Host->>ACL: ④ aclrtLoadKernel 把二进制加载进 device<br/>(拿到 kernel handle，只加载一次)
    Host->>ACL: ⑤ 准备输入数据，拷到 device 的 GM
    Host->>ACL: ⑥ aclrtlaunch_gemm_kernel(handle, args)<br/>⚠️ 只是"下发任务"，CPU 立刻返回
    ACL->>Dev2: ⑦ 把任务提交到 NPU 任务队列
    Note over Host,Dev2: ⑧ CPU 继续干别的，不等 NPU
    Dev2->>Dev2: ⑨ AI Core 执行 kernel(算 tiling→切核→搬运→Cube)
    Host->>ACL: ⑩ aclrtSynchronizeStream(stream)<br/>阻塞等待 NPU 全部完成
    ACL->>Host: ⑪ 返回(任务完成)
