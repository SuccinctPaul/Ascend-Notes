// Host: ./ascend_identity_mod <N> <launchBlocks=4096>
// launchBlocks 必须是 PARTITIONS(=32) 的倍数.
#include <iostream>
#include <vector>
#include <cmath>
#include <cstdint>
...
#include "acl/acl.h"
using half_t = __fp16;
extern "C" int aclrtlaunch_identity_mod_kernel(uint32_t, aclrtStream, void*, void*, void*, void*);
...
int main(int argc, char** argv) {
    ...
    const uint32_t NBK = (argc > 2) ? uint32_t(std::stoul(argv[2])) : 4096u;
    ...
    uint32_t tile[2] = {N, NBK};
    check("ht", aclrtMemcpy(dt, 8u, tile, 8u, ACL_MEMCPY_HOST_TO_DEVICE));
    auto t0 = std::chrono::steady_clock::now();
    int rc = aclrtlaunch_identity_mod_kernel(NBK, s, dx, dy, nullptr, dt);
    ...
    std::cout << "=== identity_mod N=" << N << " launchBlocks=" << NBK
