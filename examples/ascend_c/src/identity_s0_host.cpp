// Host: ./ascend_identity_s0 <N> <numBlocks_to_try>
// numBlocks_to_try 就是传给 aclrtlaunch_* 的 numBlocks.
#include <iostream>
#include <vector>
#include <cmath>
#include <cstdint>
...
#include "acl/acl.h"
using half_t = __fp16;
extern "C" int aclrtlaunch_identity_s0_kernel(uint32_t, aclrtStream, void*, void*, void*, void*);
...
int main(int argc, char** argv) {
    ...
    const uint32_t try_bk  = (argc > 2) ? uint32_t(std::stoul(argv[2])) : 1u;
    ...
    int rc = aclrtlaunch_identity_s0_kernel(try_bk, s, dx, dy, nullptr, dt);
    ...
    std::cout << "=== identity_s0 kernel (block 0 串行全部) N=" << N
              << "  launch_blocks=" << try_bk << "  ms=" << ms << "\n"
