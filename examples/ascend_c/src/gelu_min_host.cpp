// Host for gelu_min_kernel: ./ascend_gelu_min <N>
#include <iostream>
#include <vector>
#include <cmath>
#include <cstdlib>
...
extern "C" int aclrtlaunch_gelu_min_kernel(uint32_t, aclrtStream, void*, void*, void*, void*);
...
int main(int argc, char** argv) {
    ...
    check("createStream", aclrtCreateStream(&s));
    ...
    for (uint32_t i = 0; i < N; ++i) {
        float xv = static_cast<float>(x_h[i]);
    ...
    int rc = aclrtlaunch_gelu_min_kernel(nbk, s, dx, dy, nullptr, dt);
    ...
    for (uint32_t i = 0; i < N; ++i) {
        float a = static_cast<float>(y_ref[i]);
        float b = static_cast<float>(y_dev[i]);
        float e = std::fabs(a-b);
        if (e > mx) mx = e;
        float d = std::fmax(1e-6f, std::fabs(a)*rtol+atol);
        if (e/d > 1.f) ++bad;
    }
    std::cout << "=== gelu_min (scalar-only, no DataCopy/Vector) N=" << N << " ===\n"
              << "  kernel ms   = " << ms << "\n"
              << "  max_abs_err = " << mx << "\n"
              << "  bad         = " << bad << " / " << N << "\n"
